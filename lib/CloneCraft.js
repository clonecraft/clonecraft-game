...
class CloneCraft extends skydapp_common_1.EventContainer {
    constructor() {
        super(...arguments);
        ...
        this.currentUserAmber = ethers_1.BigNumber.from(0);
        this.clones = [];
        this.itemBalances = [];
        this.team = { units: [] };
    }
    ...
    async loadAll() {
        ...
        this.currentUserAmber = ethers_1.BigNumber.from(data.amber);
    ...
    async loadAmber() {
        this.currentUserAmber = ethers_1.BigNumber.from(await this.client.send("load-amber"));
        this.fireEvent("changeAmber");
    }
    goBattle() {
        this.world?.delete();
